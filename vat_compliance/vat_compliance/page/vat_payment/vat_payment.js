...
class VATPayment {
    ...
    make_table() {
        this.table_wrapper = $('<div style="border: 1px solid #ccc; border-radius: 5px;">').appendTo(this.page.main);
        this.datatable = null;
    }
    ...
    render_data(data) {
        ...
        let columns = [
            { name: 'Payment Date', field: 'payment_date', width: 100, format: (value) => frappe.datetime.str_to_user(value), id: 'payment_date' },
            ...
            { name: 'VAT Amount', field: 'vds_amount', width: 120, format: (value) => format_currency(value), id: 'vds_amount' },
    ...
    make_payment() {
        ...
        let selected_rows = selected_indices.map(idx => this.datatable.datamanager.data[idx]).filter(row => row && !row.is_total_row);
