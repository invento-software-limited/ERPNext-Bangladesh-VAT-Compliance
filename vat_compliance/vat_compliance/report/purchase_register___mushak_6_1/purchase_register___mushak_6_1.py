...
def get_columns():
	return [
		...
		{"label": "Invoice No", "fieldname": "name", "fieldtype": "Link",
		 "options": "Purchase Invoice", "width": 120},
		{"label": "Challan Date", "fieldname": "posting_date", "fieldtype": "Date", "width": 90},
		{"label": "Supplier Name", "fieldname": "supplier_name", "fieldtype": "Data",
		 "width": 150},
		{"label": "Supplier Address", "fieldname": "supplier_address_display", "fieldtype": "Data",
		 "width": 200},
		{"label": "BIN/NID", "fieldname": "supplier_bin_no", "fieldtype": "Data", "width": 120},
		{"label": "Description", "fieldname": "item_code", "fieldtype": "Link", "options": "Item",
		 "width": 120},
		{"label": "Quantity", "fieldname": "qty", "fieldtype": "Float", "width": 80},
		{"label": "Value (Excl. Tax)", "fieldname": "base_net_amount", "fieldtype": "Currency",
		 "width": 120},
		...
		{"label": "Total (Incl. Tax)", "fieldname": "total_amount", "fieldtype": "Currency",
		 "width": 120},
		{"label": "Stock Consumption Quantity", "fieldname": "stock_consumption_qty",
		 "fieldtype": "Float", "width": 100},
		{"label": "Stock Consumption Value", "fieldname": "stock_consumption_value",
...
def get_data(filters):
	...
	for i, row in enumerate(invoices, start=1):
		...
		if is_stock_item:
			...
			production_consumption_value = row.get('base_net_amount', 0) or row.get('net_amount',
																					0)
...
def get_stock_consumption_value(item_code, filters):
	...
	consumption_value = frappe.db.sql("""
									  ...
									  WHERE sed.item_code = %s
										...
										AND sed.s_warehouse IS NOT NULL
										AND sed.t_warehouse IS NULL
									  """, (item_code, filters.get('from_date'),
											filters.get('to_date')))
