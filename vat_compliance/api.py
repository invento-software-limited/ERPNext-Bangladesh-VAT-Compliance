...
def validate_bin(bin_no):
    ...
    try:
        ...
        frappe.log_error(f"BIN Validation Error: {str(e)}", "BIN Validation")
        frappe.throw(f"Error connecting to validation service: {str(e)}")
...
def validate_tin(tin_no):
    ...
    try:
        ...
        frappe.log_error(f"TIN Validation Error: {str(e)}", "TIN Validation")
        frappe.throw(f"Error connecting to validation service: {str(e)}")
    except Exception as e:
        frappe.log_error(f"TIN Validation Error: {str(e)}", "TIN Validation")
        frappe.throw(f"An unexpected error occurred: {str(e)}")
